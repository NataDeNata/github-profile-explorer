    ...
    plugins: [
      ...
      tailwindcss(),
      {
        ...
        configureServer(server) {
          server.middlewares.use('/api/generate-resume', (req, res) => {
            ...
            req.on('end', async () => {
              ...
              if (!apiKey) {
                res.statusCode = 500
                res.end(JSON.stringify({ error: 'Add GEMINI_API_KEY to your .env.local file.' }))
                return
              }

              try {
                const { user, repos, languages } = JSON.parse(body)
